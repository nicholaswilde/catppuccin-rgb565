...
- **context-mode**: Always leverage `context-mode` MCP tools (`ctx_execute`, `ctx_search`, `ctx_index`, etc.) for codebase exploration, code reading, and data processing.
- **codegraph**: Utilize `codegraph` (`codegraph_explore`) for semantic search, deep reasoning, and mapping relationships in this repository.
- **ponytail**: Ensure that changes are reviewed through `ponytail` (or the `ponytail-review` skill) to prevent over-engineering and encourage simplicity.
- **caveman**: Use the `caveman` suite of tools (`caveman-commit`, `caveman-review`, `cavecrew`) for all summarizations, PR reviews, commit messages, and subagent delegations to keep token usage incredibly lean.
- **rtk**: (As noted above) Always use `rtk` (Rust Token Killer) for git operations, file inspection, searches, and build outputs to ensure token-optimized communication.
